#include "pmd2_scripts.hpp"
#include <string>
#include <cassert>

using namespace std;

namespace pmd2
{
//==============================================================================
//  Constants
//==============================================================================
    namespace scriptXML
    {
        const string ATTR_ScriptExt  = "extension"; //The original file extension for the script file
        
        const string ROOT_ScriptData = "ScriptData";
        const string NODE_;

        const string ROOT_Script     = "Script";
        const string NODE_Constants  = "Constants";
        const string NODE_Strings    = "Strings";
        const string NODE_Code       = "Code";


    };

//==============================================================================
//  GameScriptsXMLParser
//==============================================================================

//==============================================================================
//  GameScriptsXMLWriter
//==============================================================================

//==============================================================================
//  GameScripts
//==============================================================================
    void GameScripts::ImportScriptsFromXML( const std::string & importdir )
    {
        assert(false);
    }

    void GameScripts::ExportScriptsToXML  ( const std::string & exportdir )
    {
        assert(false);
    }
};